"""
Java state machine code generator for CHSM.

Usage:
    render.py <html_file> [--output=<file>] [--class-name=<name>] [--package=<pkg>]

Generates a Java state machine class from a CHSM drawing (.html) file.
"""
import sys
import os
import json
...
TEMPLATE_NAME = 'chsm_java_template.jinja'
...
def render(model, template_params=None):
    """Render a Java state machine from a CHSM model dict.

    Args:
        model: CHSM model dict (as parsed from the HTML drawing).
        template_params: Optional dict of template parameters (e.g. class_name, package_name).
...
if __name__ == '__main__':
    ...
    code = render(model, template_params={'class_name': class_name, 'package_name': package_name})

...
const cevent_tst*		ccs811_events_apst[8];
ccs811_tst				ccs811_st;


ccs811_tst *self = 		&ccs811_st; 
...
static void tick_ms(uint32_t tick_cnt_u32)
{
	while(tick_cnt_u32--)
	{
		...
		CRF_POST(&tick_1ms_st, &ccs811_st);
...
void ccs811_send(chsm_tst *self, const cevent_tst *e_pst)
{
	switch(e_pst->sig)
	{
		case SIG_CCS811_ONLINE:
		case SIG_CCS811_OFFLINE:
			CRF_POST(e_pst, &q_st);
			break;
...
TEST_SETUP(ccs811)
{
    ...
    memset(&ccs811_st, 0, sizeof ccs811_st);

	cpool_init(pool_ast+0, pool_buff_au8, 24, 16);
    cqueue_init(&q_st, events_apst, 4);
	chsm_ctor(&ccs811_st.super, ccs811_top, ccs811_events_apst, 8, 0);
	

	CRF_SEND_FUNC(&ccs811_st) = ccs811_send;

	crf_init(&crf, hsm_apst, pool_ast, 1);
	chsm_init((chsm_tst *)&ccs811_st);
}

TEST_TEAR_DOWN(ccs811)
{
}
...
TEST_GROUP_RUNNER(ccs811)
{
	RUN_TEST_CASE(ccs811, init);
}

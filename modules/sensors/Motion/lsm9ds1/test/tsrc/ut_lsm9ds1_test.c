...
const cevent_tst*		lsm9ds1_events_apst[8];
lsm9ds1_tst				lsm9ds1_st;


lsm9ds1_tst *self = 		&lsm9ds1_st; 
...
static void tick_ms(uint32_t tick_cnt_u32)
{
	while(tick_cnt_u32--)
	{
		...
		CRF_POST(&tick_1ms_st, &lsm9ds1_st);
...
void lsm9ds1_send(chsm_tst *self, const cevent_tst *e_pst)
{
	switch(e_pst->sig)
	{
		case SIG_LSM9DS1_ONLINE:
		case SIG_LSM9DS1_OFFLINE:
			CRF_POST(e_pst, &q_st);
			break;
...
TEST_SETUP(lsm9ds1)
{
    ...
    memset(&lsm9ds1_st, 0, sizeof lsm9ds1_st);

	cpool_init(pool_ast+0, pool_buff_au8, 24, 16);
    cqueue_init(&q_st, events_apst, 4);
	chsm_ctor(&lsm9ds1_st.super, lsm9ds1_top, lsm9ds1_events_apst, 8, 0);
	

	CRF_SEND_FUNC(&lsm9ds1_st) = lsm9ds1_send;

	crf_init(&crf, hsm_apst, pool_ast, 1);
	chsm_init((chsm_tst *)&lsm9ds1_st);
}

TEST_TEAR_DOWN(lsm9ds1)
{
}
...
TEST_GROUP_RUNNER(lsm9ds1)
{
	RUN_TEST_CASE(lsm9ds1, init);
}
